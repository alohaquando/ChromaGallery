<script lang="ts">
	...
	async function fetchData() {
		try {
			...
			items = searchResults.items.filter((item) => !data.list.items.includes(item.id));
		} catch (error) {
...
<div class="flex justify-center items-center flex-col gap-6">
	...
	<Button
		on:click={async () => {
			await handleAddMultipleToList(data.session.uid, data.slug, selectedItems);
			await goto(`/list/${data.slug}`);
		}}
		sticky
		>Add to list
	</Button>
