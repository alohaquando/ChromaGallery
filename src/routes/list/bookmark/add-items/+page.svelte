<script lang="ts">
    ...
    let items = data.items.filter(item => !data.bookmark.items.includes(item.id));
    ...
    async function fetchData() {
        try {
            const response = await fetch(`/list/bookmark/add-items?q=${searchTerm}`);
            searchResults = await response.json(); // Store fetched search results
            items = searchResults.items.filter(item => !data.bookmark.items.includes(item.id));
        } catch (error) {
...
<div class="flex justify-center items-center flex-col gap-6">
    ...
    <Button on:click={async() => {await handleAddMultipleToList(data.session.uid, 'bookmark', selectedItems); await goto(`/list/bookmark`)}}
            sticky>Add to list
    </Button>
</div>
