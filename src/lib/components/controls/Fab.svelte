...
<script lang="ts">
	...
	let hoverEffect = '';
	if (hover) {
		hoverEffect = 'hover:before:opacity-100 hover:after:opacity-100 hover:text-black hover:bg-white/30';
	}
...
<svelte:element
	class="{customClasses} {sizeClasses} {hoverEffect} text-center flex-col  rounded-full border border-white border-opacity-30 backdrop-blur-sm justify-center items-center gap-2 inline-flex overflow-visible duration-300
