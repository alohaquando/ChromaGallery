<script>
  ...
  export let type = Type.Actionable;
  export let state = State.Enabled;
</script>

<button class="px-3 py-2 rounded-full backdrop-blur-sm justify-center items-center gap-1.5 inline-flex line-clamp-1 overflow-visible ease-out duration-300 text-sm text-center leading-[14px]
        {type === Type.Actionable ? 'hover:bg-white/60 hover:text-gray-900 hover:before:opacity-70 hover:after:opacity-70' : 'bg-gray-900 rounded-lg text-white'}
